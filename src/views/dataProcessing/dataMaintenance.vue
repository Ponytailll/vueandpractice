    ...
    <div class="el-card dataMain-comPad">
      ...
      <div style="">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item :to="{ path: '/personalCenter/Dashboard' }">首页</el-breadcrumb-item>
          <el-breadcrumb-item><a :to="{ path: '/personalCenterDashboard/dataMaintenance/dataMain' }">数据维护</a>
          </el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!--按钮-->
      <div>
        ...
        <el-button :disabled="btnDisFlag" class="buttom-common" type="primary" size="mini" @click="openCompany">导出公司<i
          class="el-icon-download" style="padding-left: 5px;"></i></el-button> <!--class="icon-cloud-download"-->
        <el-button :disabled="btnDisFlag" class="buttom-common" type="primary" size="mini" @click="exportTitle">导出详细<i
      ...
      <el-tabs v-model="editableTabsValue" type="card" editable @edit="handleTabsEdit" @tab-click="handleClick"
        ...
        <el-tab-pane v-for="(item, index) in editableTabs" :label="item.labelName" :name="item.labelId"
          ...
          <el-table v-loadmore="loadMore" :data="recordImportList" max-height="630" style="width: 100%;"
            ...
            <el-table-column prop="companyName" :filters="recordFilter.recordCompanyList" :filter-method="filterHandler"
...
//        let exportClassificationList = that.$refs.tree.getCheckedKeys();
        ...
        if (exportClassificationList === undefined || exportClassificationList == null || exportClassificationList === "" || exportClassificationList.length === 0) {
          flag = "noExport";
        } else if (that.recordImportList === undefined || that.recordImportList == null || that.recordImportList === "" || that.recordImportList.length === 0) {
          flag = "noCompany"
        }
        if (flag === "noCompany") {
          that.$message({
